...
## M10.0 先追一條線：新增 MMIO Timer peripheral

讀 Chipyard/Rocket code 時，最容易迷失的原因是同一個功能被拆在 config、graph、RTL、metadata、
harness 多個位置。不要先背所有 API；先追一個 peripheral 從「是否存在」到「軟體看得到」的完整路徑。

以 Timer peripheral 為例，真正要回答的是這七個問題：

```text
1. 這個 SoC variant 要不要 Timer？        -> Config fragment / Key
2. Timer 的 address/width/interrupt 是什麼？ -> Params
3. 它掛在哪條 bus？                       -> LazyModule / TileLink node
4. CPU 如何 read/write register？          -> TLRegisterNode / regmap / RegField
5. interrupt 如何接到 CPU/PLIC？           -> IntSourceNode / interrupt graph
6. Linux/軟體如何知道它存在？              -> SimpleDevice / device tree metadata
7. 模擬專用 timeout/monitor 放哪？          -> HarnessBinder / TestHarness
```

這條線就是本章的主線。後面的 Config、`Parameters`、`LazyModule`、Diplomacy、TileLink widget、
RegMapper、device tree、binder 都是在解這條線上的某個問題。讀任何 Chipyard/Rocket 片段時，
先問「我現在站在哪一層？」比先查語法細節更重要。

對應到本章後續小節：

| Timer 路徑問題 | 後續小節 |
|----------------|----------|
| variant 是否啟用、參數從哪來 | M10.3–M10.5 |
| graph 如何連、bus 能力如何協商 | M10.6–M10.10 |
| MMIO register 與 side effect | M10.11 |
| device tree / interrupt / crossing | M10.12–M10.14 |
| harness / bridge / cosim | M10.15–M10.16、M10.20 |

---

## M10.1 先拆 Scala 語法：讀 Chipyard 前的最低門檻
...
## M10.3 Config fragments（像 recipe 疊出一顆 SoC）
...
## M10.5 `site` / `here` / `up`（config fragment 解析）
...
## M10.6 `LazyModule`：two-phase elaboration
...
## M10.7 Diplomacy：parameter negotiation framework
...
## M10.8 TileLink nodes 與 connection operators
...
## M10.11 MMIO peripheral：`TLRegisterRouter` / `regmap` / `RegField`
...
### 詳細案例：新增一個 MMIO Timer peripheral
...
## M10.12 Device tree 與 interrupt nodes
...
## M10.13 Clock/Reset crossing framework
...
## M10.14 Cake pattern / traits 與 `CanHavePeripheryXXX`
...
## M10.15 IOBinders / HarnessBinders（SoC IO 與 harness 解耦）
...
## M10.16 RoCC accelerator 與 TraceIO cosim
...
## M10.17 Tile / Subsystem / ChipTop 分層
...
## M10.20 如何讀一段 Chipyard/Rocket 程式碼
