...
| 看到的寫法 | 先問什麼 |
|------------|----------|
| `UInt(8.W)` / `Output(UInt(...))` | 這條跨模組邊界的寬度是否明確 |
| `Bundle` | 這是 payload、protocol，還是 top-level IO |
| `WireDefault` / `otherwise` | 每條組合輸出在所有路徑都有值嗎 |
| `RegInit` / `RegEnable` | reset 值與 enable 條件是否是設計契約 |
| `Vec` / `Seq` | 需要硬體索引，還是只是生成期集合 |
...
```scala
// 標記：概念示意。重點是 width 與 literal 寫法。
...
```scala
// 標記：教學骨架。可跑的語意化 ALU 見 ../examples/src/main/scala/handout/Combinational.scala。
// ✗ 散亂
...
```scala
// 標記：教學骨架。payload bundle 不帶方向，方向交給使用點。
// ✓ payload：純資料，不含方向
...
```scala
// 標記：概念示意。用來展示組合 default 與 last-connect-wins。
